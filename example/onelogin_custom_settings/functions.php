...
define('SPIDCODENAME', 'spidCode');	#ITWallet: defining constant for spidCode name in SAML response
...
function saml_acs() {
	...
	if (!empty($attrs)) {
		...
		$spidCodeMapping = 'spidCodes'; #ITWallet: added uri option non nameidformat droplist
		...
		#ITWallet: adding spidCode to the user object
		if (!empty($spidCodeMapping) && isset($attrs[$spidCodeMapping])  && !empty($attrs[$spidCodeMapping][0])){
			$userdata[SPIDCODENAME] = $attrs[$spidCodeMapping][0];
		}
	...
	if ($user_id) {
		...
		if (get_option('onelogin_saml_updateuser')) {
			...
			#ITWallet: updating spidCode value on the db for the existing user
			update_user_meta($user_id, SPIDCODENAME, $userdata[SPIDCODENAME]);
		}
		...
		if ($user_id && !is_a($user_id, 'WP_Error')) {
			...
			#ITWallet: adding spidCode value on the db for new user
			update_user_meta($user_id, SPIDCODENAME, $userdata[SPIDCODENAME]);
		}
	} else {

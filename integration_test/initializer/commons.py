...
def set_credential_issuer_jwk_conf(credential_issuer_jwk: JWK):
    global _CREDENTIAL_ISSUER_JWK_VALUE
    _CREDENTIAL_ISSUER_JWK_VALUE = credential_issuer_jwk

def get_credential_issuer_jwk_conf():
    if _CREDENTIAL_ISSUER_JWK_VALUE is None:
        return CREDENTIAL_ISSUER_JWK
    return _CREDENTIAL_ISSUER_JWK_VALUE
...
_ISSUER_CONF_VALUE = None

def set_issuer_conf(issuer_conf: dict):
    global _ISSUER_CONF_VALUE
    _ISSUER_CONF_VALUE = issuer_conf

def get_issuer_conf():
    if _ISSUER_CONF_VALUE is None:
        return ISSUER_CONF
    return _ISSUER_CONF_VALUE

CREDENTIAL_ISSUER_TRUST_SOURCE_Dict = {
    "entity_id": get_issuer_conf()["issuer"],
    "policies": {},
    "metadata": {},
    "revoked": False,
    "direct_trust_sd_jwt_vc": {
        ...
        "jwks": [get_credential_issuer_jwk_conf().as_dict()],
...
def create_issuer_test_data_with_user_claims(user_claims:dict) -> dict[Literal["jws"] | Literal["issuance"], str]:
    ...
    issued_jwt = SDJWTIssuer(
        issuer_keys=get_credential_issuer_jwk_conf().as_dict(),
        holder_key=public_holder_key,
        extra_header_parameters={
            "typ": "dc+sd-jwt",
            "kid": get_credential_issuer_jwk_conf().kid
        },
...
def extract_content_title_login_page(page_content: str) -> PageElement:
    bs = BeautifulSoup(page_content, features="html.parser")
    return list(bs.find(id="content-title").children)[0]

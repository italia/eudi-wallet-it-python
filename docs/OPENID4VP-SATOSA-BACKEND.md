...
2. copy and customize [pyeudiw_backend.yaml](integration_test/conf/pyeudiw_backend.yaml)
3. copy and customize the content of the folders [static](integration_test/static) and [templates](integration_test/templates) to your satosa deployment.
4. include the backend configuration in your satosa configuration
5. customize the file `internal_attributes.yaml` used in your deployment, enabling the `openid4vp` protocol. See [example/satosa/internal_attributes.yaml](integration_test/conf/internal_attributes.yaml) as example.
6. start Satosa.

## Backend configuration

1. Customize [pyeudiw_backend.yaml](integration_test/conf/pyeudiw_backend.yaml), then copy it in your satosa `plugins/backend` project folder. Example `plugins/backends/pyeudiw_backend.yaml`;
...
5. Customize [static/disco.html](integration_test/static/disco.html), then copy it in satosa static file folder. Example `example/static/static/disco.html`
6. Customize [templates/*.html](integration_test/templates/*.html), then copy it in satosa templates file folder (the path your have configured in your `pyeudiw_backend.yaml` file).
7. Customize [internal_attributes.yaml](integration_test/conf/internal_attributes.yaml), then copy it the path your have configured in your `proxy_conf.yaml` file.
...
| Key                                       | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
|-------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| config.security.accepted_referer_criteria | An optional list of regular expressions that are matched against the HTTP_REFERER header of an incoming request. <br/> If the referer matches one of these regex patterns, the system will force the same-device flow (SAME_DEVICE) even if the client is on a desktop or non-mobile device. <br/> This enables use cases where a user already has a credential in their wallet and is scanning a QR code to obtain a new credential, allowing authentication using the existing PID without requiring a cross-device flow. |

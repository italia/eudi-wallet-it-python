...
| config.default_target_authentication_backend   | Is the primary authentication backend used by the system when no alternative backend is specified. It is responsible for managing the entire user authentication process with the configured Identity Provider(s) (IdPs), default value is `openid4vp`                                                                                                                                                   |

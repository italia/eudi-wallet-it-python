...
2. copy and customize [openid4vci_frontend.yaml](integration_test/conf/openid4vci_frontend.yaml)
...
4. customize the file `internal_attributes.yaml` used in your deployment, enabling the `openid4ci` protocol.
   See [internal_attributes.yaml](integration_test/conf/internal_attributes.yaml) as example.
5. start Satosa.

## Frontend configuration

1. Customize [openid4vci_frontend.yaml](integration_test/conf/openid4vci_frontend.yaml), then copy it in your
...
2. Add `  - "plugins/frontend/openid4vci_frontend.yaml"` in your SATOSA `proxy_conf.yaml` file, within the section
   `FRONTEND_MODULES`;
3. Customize [internal_attributes.yaml](integration_test/conf/internal_attributes.yaml), then copy it the path
...
| Key                                         | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
|---------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| config.security.dpop_required               | This is the flag to enable the DPOP validation needed as access token for token and credential endpoints<br />([https://datatracker.ietf.org/doc/html/rfc9449](https://datatracker.ietf.org/doc/html/rfc9449)).<br /> If this setting is missing it will be set to true by default.                                                                                                                                                                                                                                         |
...
| config.security.proof_jwt_required          | If true, the credential endpoint requires a proof JWT in the request; if false or missing (default), proof is optional and a missing proof is logged at debug level.                                                                                                                                                                                                                                                                                                                                                    |
| config.security.signed_par_request          | This flag is used to enable the signed par request for interoperability, if missing it will be set to "true" by default.<br />Can be "true", "false" or "both".<br />If both is selected the Front End will try to search for the field request and, if is not present, will try to retrive the info <br />from the url encoded request.                                                                                                                                                                                    |
...
| config.default_target_authentication_backend   | It is the primary authentication backend used by the system when no alternative backend is previously selected by User. Implementations using legacy infrastructure of authenticate the Users for the issuance of their PIDs will rely on preexisting backends, while the implementantions authenticating Users through a Credential Presentation will set this to `openid4vp`.                                             |

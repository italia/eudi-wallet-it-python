...
def _gen_ec(node: LeafInfo | IntermediateInfo | TrustedAnchorInfo, keys: list) -> dict:
    ec = {
		'exp': exp_from_now(node.exp_in), # TODO: get all the timestamp dinamically
		'iat': iat_now(),
		'iss': node.issuer,
		'sub': node.subject,
    ...
    if is_leaf:
        ...
        ec["metadata"]["openid_relying_party"] = {"jwks": _gen_empty_jwks_field()}
    ...
    for key in keys:
        ...
        if is_leaf:
            ec['metadata']['openid_relying_party']["jwks"]['keys'].append(serialized_key)

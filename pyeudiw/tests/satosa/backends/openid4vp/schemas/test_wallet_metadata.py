...
def test_alg_values_supported_valid(value):
    ...
    metadata = WalletMetadata(
        vp_formats_supported=_example_vp_formats_supported, alg_values_supported=value
    )
...
def test_valid_authorization_endpoint():
    ...
    wallet_post_request_validate = WalletPostRequest.model_validate(request)
    assert (
        wallet_post_request_validate.wallet_metadata.authorization_endpoint
        == valid_auth_endpoint
    )

    wallet_post_request = WalletPostRequest(**request)
    assert (
        wallet_post_request.wallet_metadata.authorization_endpoint
        == valid_auth_endpoint
    )
...
@pytest.mark.parametrize(
    "value", ["  ", "invalid_authorization_endpoint", "https://example.com"]
)
...
@pytest.mark.parametrize(
    "value", [["direct_post_jwt"], "direct_post_jwt", ["direct_post_jwt", "test"], []]
)
def test_valid_response_mode_with_context(value):
    ...
    wallet_post_request_validate = WalletPostRequest.model_validate(
        request, context=ctx
    )
    assert wallet_post_request_validate.wallet_metadata.response_modes_supported == [
        "direct_post_jwt"
    ]
...
@pytest.mark.parametrize(
    "value",
    [["direct_post_jwt"], "direct_post_jwt", ["direct_post_jwt", "test"], None, []],
)
def test_valid_response_mode_without_context(value):
    ...
    wallet_post_request_validate = WalletPostRequest.model_validate(request)
    assert (
        wallet_post_request_validate.wallet_metadata.response_modes_supported
        == expected_value
    )

    wallet_post_request = WalletPostRequest(**request)
    assert (
        wallet_post_request.wallet_metadata.response_modes_supported == expected_value
    )
...
@pytest.mark.parametrize(
    "value", [["vp_token"], "vp_token", ["vp_token", "test"], [], None]
)
def test_valid_response_types_supported(value):
    ...
    assert (
        WalletPostRequest.model_validate(
            request
        ).wallet_metadata.response_types_supported
        == expected_value
    )
    assert (
        WalletPostRequest(**request).wallet_metadata.response_types_supported
        == expected_value
    )
    assert (
        WalletMetadata.model_validate(
            request["wallet_metadata"]
        ).response_types_supported
        == expected_value
    )
    assert (
        WalletMetadata(**request["wallet_metadata"]).response_types_supported
        == expected_value
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {
                "dc+sd-jwt": {"sd-jwt_alg_values": ["ES256"]},
                "mso_mdoc": {"mso_mdoc_alg_values": ["ES256"]},
            },
            {
                "dc+sd-jwt": {"sd-jwt_alg_values": ["ES256"]},
                "mso_mdoc": {"mso_mdoc_alg_values": ["ES256"]},
...
def test_valid_vp_formats_context(value, expected_value):
    ...
    assert (
        WalletPostRequest.model_validate(
            request, context=ctx
        ).wallet_metadata.vp_formats_supported
        == expected_value
    )
    assert (
        WalletMetadata.model_validate(
            request["wallet_metadata"], context=ctx
...
def test_invalid_vp_formats_supported_with_context():
    request = {
        "wallet_metadata": {
            "vp_formats_supported": {"other": {"other_alg_values": ["ES256"]}}
        }
    }
    ctx = {VP_FORMATS_SUPPORTED_CTX: ["dc+sd-jwt", "mso_mdoc", "jwt_vc_json"]}
    with pytest.raises(ValidationError) as err:
...
def test_valid_vp_formats_supported_without_context(value):
    ...
    assert (
        WalletPostRequest.model_validate(request).wallet_metadata.vp_formats_supported
        == value
    )
    assert WalletPostRequest(**request).wallet_metadata.vp_formats_supported == value
    assert (
        WalletMetadata.model_validate(request["wallet_metadata"]).vp_formats_supported
        == value
    )
...
def test_valid_client_id_schemes_supported_with_context(value, expected_value):
    ...
    assert (
        WalletPostRequest.model_validate(
            request, context=ctx
        ).wallet_metadata.client_id_schemes_supported
        == expected_value
    )
    assert (
        WalletMetadata.model_validate(
...
def test_valid_client_id_schemes_supported_without_context(value):
    ...
    assert (
        WalletPostRequest(**request).wallet_metadata.client_id_schemes_supported
        == expected_value
    )
    assert (
        WalletMetadata.model_validate(
    ...
    assert (
        WalletMetadata(**request["wallet_metadata"]).client_id_schemes_supported
        == expected_value
    )
...
def test_valid_request_obj_sig_alg_values_supported_with_context(value, expected_value):
    ...
    assert (
        WalletPostRequest.model_validate(
            request, context=ctx
        ).wallet_metadata.request_object_signing_alg_values_supported
        == expected_value
    )
    assert (
        WalletMetadata.model_validate(
...
def test_invalid_request_obj_sig_alg_values_supported_with_context(value):
    ...
    with pytest.raises(ValidationError) as err:
        WalletPostRequest.model_validate(request, context=ctx)
    assert "Invalid value for request_object_signing_alg_values_supported" in str(
        err.value
    )
    with pytest.raises(ValidationError) as err:
        WalletMetadata.model_validate(request["wallet_metadata"], context=ctx)

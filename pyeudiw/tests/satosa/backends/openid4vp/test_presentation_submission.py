...
from pyeudiw.tests.satosa.backends.openid4vp.mock_parser_handlers import MockLdpVpHandler, MockJwtVpJsonHandler, MockFailingParser
...
mock_format_config = {
    "formats": [
        {"format": "ldp_vp", "module": "pyeudiw.tests.satosa.backends.openid4vp.mock_parser_handlers", "class": "MockLdpVpHandler"},
        {"format": "jwt_vp_json", "module": "pyeudiw.tests.satosa.backends.openid4vp.mock_parser_handlers", "class": "MockJwtVpJsonHandler"},
        {"format": "fail_parser", "module": "pyeudiw.tests.satosa.backends.openid4vp.mock_parser_handlers", "class": "MockFailingParser"},
    ],
...
valid_submission = {
    ...
    "descriptor_map": [{"id": "descriptor_1", "format": "ldp_vp", "path": "$[0]"}, {"id": "descriptor_2", "format": "jwt_vp_json", "path": "$[1]"}],
}
...
def ps():
    handler_config = load_credential_presentation_handlers(
        {"credential_presentation_handlers": mock_format_config}, trust_evaluator=trust_ev, sig_alg_supported=["ES256", "ES384", "ES512"]
    )
...
def test_handler_parser_failure(ps):
    invalid_submission = {
        "id": "submission_id",
        "definition_id": "definition_id",
        "descriptor_map": [{"id": "descriptor_1", "format": "fail_parser", "path": "$[0]"}, {"id": "descriptor_2", "format": "jwt_vp_json", "path": "$[1]"}],
    }
    with pytest.raises(Exception) as exc_info:

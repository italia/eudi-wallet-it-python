...
def get_valid_context(
    ...
    MOCK_PYEUDIW_FRONTEND_CONFIG["metadata"]["oauth_authorization_server"][
        "scopes_supported"
    ] = scopes_supported
...
def test_token_request_valid_refresh_token_grant():
    ...
    req = TokenRequest.model_validate(
        payload,
        context=get_valid_context(scopes_supported=["scope1", "scope2", "openid"]),
    )
...
def test_token_request_invalid_scope():
    ...
    with pytest.raises(
        InvalidRequestException, match="invalid scope value 'unknown_scope'"
    ):
        TokenRequest.model_validate(
            payload, context=get_valid_context(scopes_supported=["scope1", "scope2"])
        )

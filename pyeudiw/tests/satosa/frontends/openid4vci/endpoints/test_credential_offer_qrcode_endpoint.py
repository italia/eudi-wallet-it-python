...
from pyeudiw.satosa.frontends.openid4vci.endpoints.credential_offer_qrcode_endpoint import (
    CredentialOfferQrCodeHandler,
)
...
_CREDENTIAL_OFFER_QRCODE_BASE_PATH = (
    f"{BASE_PACKAGE}.endpoints.credential_offer_qrcode_endpoint"
)


@pytest.fixture
def credential_offer_qrcode_handler() -> CredentialOfferQrCodeHandler:
    with patch(f"{_CREDENTIAL_OFFER_QRCODE_BASE_PATH}.Jinja2TemplateHandler"):
        return CredentialOfferQrCodeHandler(
            MOCK_PYEUDIW_FRONTEND_CONFIG,
            MOCK_INTERNAL_ATTRIBUTES,
            MOCK_BASE_URL,
            MOCK_NAME,
...
@pytest.mark.parametrize(
    ...
    [
        (
            mock_deserialized_overridable(
                MOCK_PYEUDIW_FRONTEND_CONFIG, {"qrcode": REMOVE}
            ),
            ["qrcode"],
        ),
        (
            mock_deserialized_overridable(
                MOCK_PYEUDIW_FRONTEND_CONFIG, {"qrcode.size": REMOVE}
            ),
            ["qrcode.size"],
        ),
        (
            mock_deserialized_overridable(
                MOCK_PYEUDIW_FRONTEND_CONFIG, {"qrcode.color": REMOVE}
            ),
            ["qrcode.color"],
        ),
        (
            mock_deserialized_overridable(
                MOCK_PYEUDIW_FRONTEND_CONFIG, {"qrcode.expiration_time": REMOVE}
            ),
            ["qrcode.expiration_time"],
        ),
        (
            mock_deserialized_overridable(
                MOCK_PYEUDIW_FRONTEND_CONFIG, {"qrcode.logo_path": REMOVE}
            ),
            ["qrcode.logo_path"],
        ),
        (
            mock_deserialized_overridable(
                MOCK_PYEUDIW_FRONTEND_CONFIG, {"qrcode.ui": REMOVE}
            ),
            ["qrcode.ui"],
        ),
        (
            mock_deserialized_overridable(
                MOCK_PYEUDIW_FRONTEND_CONFIG, {"qrcode.ui.static_storage_url": REMOVE}
            ),
            ["qrcode.ui.static_storage_url"],
        ),
        (
            mock_deserialized_overridable(
                MOCK_PYEUDIW_FRONTEND_CONFIG, {"qrcode.ui.template_folder": REMOVE}
            ),
            ["qrcode.ui.template_folder"],
        ),
        (
            mock_deserialized_overridable(
                MOCK_PYEUDIW_FRONTEND_CONFIG, {"qrcode.ui.qrcode_template": REMOVE}
            ),
            ["qrcode.ui.qrcode_template"],
        ),
        (
            mock_deserialized_overridable(
                MOCK_PYEUDIW_FRONTEND_CONFIG,
                {"qrcode.ui.authorization_error_template": REMOVE},
            ),
            ["qrcode.ui.authorization_error_template"],
        ),
        (
            mock_deserialized_overridable(
...
def test_missing_configurations(config, missing_fields):
    do_test_missing_configurations_raises(
        CredentialOfferQrCodeHandler, config, missing_fields
    )

...
_MOCK_PAR_REQUEST = {"request": "request.valid.jwt", "client_id": _MOCK_VALID_THUMBPRINT}
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            _mock_configuration_removed_oauth_authorization_server_param("response_types_supported"),
            ["metadata.oauth_authorization_server.response_types_supported"],
        ),
        (
            _mock_configuration_removed_oauth_authorization_server_param("response_modes_supported"),
            ["metadata.oauth_authorization_server.response_modes_supported"],
        ),
        (
            _mock_configuration_removed_oauth_authorization_server_param("code_challenge_methods_supported"),
        ...
        (
            ...
            ["jwt.par_exp", "metadata.openid_credential_issuer.credential_configurations_supported"],
        ),
    ],
)
...
def test_invalid_oauth_client_attestation(par_handler, headers):
    ...
    assert_invalid_request_application_json(par_handler.endpoint(context), "Missing Wallet Attestation JWT header")
...
@pytest.mark.parametrize(
    ...
    [
            ...
            _mock_request_deserialized(
                {
                    ...
                    "exp": int(datetime.datetime.now(datetime.timezone.utc).timestamp()) + 329,
                }
            ),
            "expired token",
        ),
        ...
        (
            _mock_request_deserialized({"authorization_details": [{"type": OPEN_ID_CREDENTIAL_TYPE}]}),
            "missing `authorization_details.credential_configuration_id` parameter",
        ),
        (
            _mock_request_deserialized({"authorization_details": [{"credential_configuration_id": "dc_sd_jwt_EuropeanDisabilityCard"}]}),
            "missing `authorization_details.type` parameter",
        ),
        (
            _mock_request_deserialized(
                {"authorization_details": [{"type": OPEN_ID_CREDENTIAL_TYPE, "credential_configuration_id": "invalid_credential_configuration_id"}]}
            ),
            "invalid `authorization_details.credential_configuration_id` parameter",
        ),
        (
            _mock_request_deserialized(
                {"authorization_details": [{"type": "invalid_credential_type", "credential_configuration_id": "dc_sd_jwt_EuropeanDisabilityCard"}]}
            ),
            "invalid `authorization_details.type` parameter",
        ),
    ],
)
def test_invalid_request_deserialized(par_handler, context, decoded_request, error_desc):
    with (
        ...
        assert_invalid_request_application_json(par_handler.endpoint(context), error_desc)

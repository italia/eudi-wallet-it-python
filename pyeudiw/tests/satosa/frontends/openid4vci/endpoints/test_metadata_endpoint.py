...
def _get_entity_configuration_payload(
    metadata_handler: MetadataHandler, context: Context, *, as_json: bool
) -> dict:
...
def _collect_all_jwk_dicts_from_entity_config(payload: dict) -> list[dict]:
    ...
    if (
        "jwks" in payload
        and isinstance(payload["jwks"], dict)
        and "keys" in payload["jwks"]
    ):
        keys_list.extend(payload["jwks"]["keys"])
    for metadata_val in payload.get("metadata", {}).values():
        if (
            isinstance(metadata_val, dict)
            and "jwks" in metadata_val
            and isinstance(metadata_val["jwks"], dict)
        ):
...
def _assert_no_private_key_material(jwk_dict: dict) -> None:
    ...
    for param in PRIVATE_KEY_PARAMS:
        assert (
            param not in jwk_dict
        ), f"Private key parameter {param!r} must not appear in published metadata"
...
def context() -> Context:
    return get_mocked_satosa_context(
        method="GET", headers={HTTP_CONTENT_TYPE_HEADER: APPLICATION_JSON}
    )
...
def _mock_configurations(field: list[str] | str):
    ...
    if field != "credential_configurations":
        ...
        [
            credential_configurations.pop(f, None)
...
@pytest.mark.parametrize(
    ...
    [
        (
            _mock_configurations("credential_configurations"),
...
def test_endpoint_returns_json_with_ensured_credential_issuer(
    metadata_handler, context
):
    _run_endpoint_returns_json_test(
        metadata_handler, context, _DEFAULT_ISSUER_FROM_STANDARD_CONFIG
    )
...
def _run_endpoint_returns_json_test(metadata_handler, context, expected_issuer: dict):
    ...
    _assert_metadata(
        metadata_handler.config, response_data["metadata"], expected_issuer
    )
...
def _assert_metadata(config: dict, response_metadata: dict, expected_issuer: dict):
    ...
    assert (
        response_metadata["openid_credential_issuer"]["credential_issuer"]
        == expected_issuer["openid_credential_issuer"]
    )
    assert (
        response_metadata["oauth_authorization_server"]["issuer"]
        == expected_issuer["oauth_authorization_server_issuer"]
    )


@pytest.mark.parametrize("as_json", [True, False])
def test_entity_configuration_jwks_contain_no_private_keys(
    metadata_handler, context, as_json: bool
):
    """All JWKs in published entity configuration must be public only; private key material must never be exposed."""
    payload = _get_entity_configuration_payload(
        metadata_handler, context, as_json=as_json
    )
    for jwk_dict in _collect_all_jwk_dicts_from_entity_config(payload):
        _assert_no_private_key_material(jwk_dict)


def test_entity_configuration_federation_jwks_count_and_public_only(
    metadata_handler, context
):
    ...
    expected_count = len(
        MOCK_PYEUDIW_FRONTEND_CONFIG["trust"]["federation"]["config"].get(
            "federation_jwks", []
        )
    )
    ...
    assert (
        len(entity_keys) == expected_count
    ), f"Expected {expected_count} federation public key(s) in entity jwks, got {len(entity_keys)}"
    for jwk_dict in entity_keys:
        _assert_no_private_key_material(jwk_dict)


def test_metadata_credential_issuer_jwks_contain_no_private_keys_when_config_has_jwks(
    context,
):

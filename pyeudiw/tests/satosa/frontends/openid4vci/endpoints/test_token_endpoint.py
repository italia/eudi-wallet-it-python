...
from pyeudiw.satosa.frontends.openid4vci.endpoints.token_endpoint import (
    TokenHandler,
    TokenTypsEnum,
)
...
def _mock_configurations(field: str):
    ...
    match field:
        ...
        case "oauth_authorization_server.scopes_supported":
            oauth_authorization_server = deepcopy(
                MOCK_OAUTH_AUTHORIZATION_SERVER_CONFIG
            )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            _mock_configurations("oauth_authorization_server"),
            ["metadata.oauth_authorization_server"],
        ),
        (
            _mock_configurations("oauth_authorization_server.scopes_supported"),
        ...
        (
            _mock_configurations("access_token_exp,refresh_token_exp"),
            ["jwt.access_token_exp", "jwt.refresh_token_exp"],
        ),
    ],
)
...
def test_invalid_request_code_with_grant_type_authorization_code(
    ...
    with patch("pyeudiw.jwt.jws_helper.JWSHelper.verify", return_value=None):
        ...
        _assert_invalid_request(
            token_handler.endpoint(context), "missing `code` parameter"
        )


def test_invalid_request_code_with_grant_type_refresh_token(
...
def test_invalid_request_redirect_uri_with_grant_type_authorization_code(
...
def test_invalid_request_redirect_uri_with_grant_type_refresh_token(
    token_handler, context, valid_request_refresh_token
):
    with patch("pyeudiw.jwt.jws_helper.JWSHelper.verify", return_value=None):
        token_handler.db_engine.get_by_session_id.return_value = (
...
def test_invalid_request_code_verifier_with_grant_type_authorization_code(
    token_handler, context, valid_request_authorization_code, value, err_descr
):
    with patch("pyeudiw.jwt.jws_helper.JWSHelper.verify", return_value=None):
        token_handler.db_engine.get_by_session_id.return_value = (
...
def test_invalid_request_code_verifier_with_grant_type_refresh_token(
    token_handler, context, valid_request_refresh_token
):
    with patch("pyeudiw.jwt.jws_helper.JWSHelper.verify", return_value=None):
        token_handler.db_engine.get_by_session_id.return_value = (
...
def test_invalid_refresh_token_with_grant_type_c(
    ...
    with patch("pyeudiw.jwt.jws_helper.JWSHelper.verify", return_value=None):
        ...
        _assert_invalid_request(
            token_handler.endpoint(context), "unexpected `refresh_token` parameter"
        )


@pytest.mark.parametrize("value", ["", None, " "])
def test_invalid_request_refresh_token_with_grant_type_refresh_token(
...
def test_invalid_scopes_with_grant_type_authorization_code(
    token_handler, context, valid_request_authorization_code
):
    with patch("pyeudiw.jwt.jws_helper.JWSHelper.verify", return_value=None):
        token_handler.db_engine.get_by_session_id.return_value = (
        ...
        _assert_invalid_request(
            token_handler.endpoint(context), "unexpected `scope` parameter"
        )
...
def test_valid_request_with_grant_type_authorization_code(
    ...
    _assert_test_valid_request_with_grant_type_authorization_code(
        context, token_handler, valid_request_authorization_code
    )
...
def test_valid_with_request_with_grant_type_authorization_code_and_invalid_oauth_client_attestation_with_dpop_disabled(
    headers, valid_request_authorization_code
):
    ...
    headers["HTTP_USER_AGENT"] = (
        "Mozilla/5.0 (Linux; Android 10; SM-G960F) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/77.0.3865.92 Mobile Safari/537.36"
    )
...
def _assert_test_valid_request_with_grant_type_authorization_code(
    context: Context,
    token_handler: TokenHandler,
    valid_request_authorization_code: dict,
):
    with (
        patch("pyeudiw.jwt.jws_helper.JWSHelper.sign", side_effect=mock_sign),
...
def test_valid_with_request_with_grant_type_refresh_token_and_invalid_oauth_client_attestation_with_dpop_disabled(
    headers, valid_request_authorization_code
):
    token_handler = TokenHandler(
        mock_deserialized_overridable(
...
def test_valid_with_request_with_dpop_enabled(valid_request_authorization_code):
    token_handler = TokenHandler(
        mock_deserialized_overridable(
            MOCK_PYEUDIW_FRONTEND_CONFIG,
            {"security": {"dpop_required": True, "wallet_attestation_required": False}},
        ),
        MOCK_INTERNAL_ATTRIBUTES,
        MOCK_BASE_URL,
        MOCK_NAME,
...
def _assert_test_valid_request_with_grant_type_refresh_token(
    context: Context, token_handler: TokenHandler, valid_request_refresh_token: dict
):
    with (
        patch("pyeudiw.jwt.jws_helper.JWSHelper.sign", side_effect=mock_sign),
        patch("pyeudiw.jwt.jws_helper.JWSHelper.verify", return_value=None),
...
def _assert_valid_request(
    result: Response, entity: dict, exp_access_token: str, exp_refresh_token: str
):
    ...
    assert (
        ...
        if not entity["authorization_details"]
        and len(entity["authorization_details"]) == 0
...
def _assert_invalid_request(result: Response, error_desc: str):
    ...
    assert (
        result.message
        == f'{{"error": "invalid_request", "error_description": "{error_desc}"}}'
    )

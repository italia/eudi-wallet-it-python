...
def trust_evaluator():
    return CombinedTrustEvaluator.from_config(
        correct_config, DBEngine(CONFIG["storage"]), default_client_id="test-client"
    )
...
def test_missing_credential_presentation_handlers_raises(trust_evaluator):
    ...
    with pytest.raises(ValueError) as exc:
        load_credential_presentation_handlers(
            config={}, trust_evaluator=trust_evaluator
        )
...
def test_missing_class_raises(trust_evaluator):
    ...
    with pytest.raises(ImportError) as exc:
        load_credential_presentation_handlers(
            config=invalid_config, trust_evaluator=trust_evaluator
        )
...
def test_class_not_subclass_of_base_raises(trust_evaluator):
    ...
    assert (
        "Class 'NotASubclass' not found in module 'pyeudiw.tests.satosa.backends.openid4vp.mock_parser_handlers' for format 'not_subclass'."
        in str(exc.value)
    )
...
def test_empty_formats_raises(trust_evaluator):
    invalid_config = {
        "credential_presentation_handlers": {"max_submission_size": 2048, "formats": []}
    }

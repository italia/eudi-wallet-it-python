...
class RequestHandler(VPBaseEndpoint):
    ...
    def __init__(
        ...
        self._client_id_schemes_supported = (
            client_id_schemes if client_id_schemes else None
        )
    ...
    def endpoint(self, context: Context) -> Response:
        ...
        if context.request_method == "POST":
            try:
                wallet_post_request = WalletPostRequest.model_validate(
                    request,
                    context={
                        RESPONSE_MODES_SUPPORTED_CTX: self.config["authorization"].get(
                        ...
                        REQUEST_OBJ_SIG_ALG_VALUES_SUPPORTED: self.config["jwt"].get(
                            "sig_alg_supported"
                        ),
                    },
                )
                ...
                self._log_warning(
                    context, f"wallet metadata not provided or invalid: {e}"
                )
        ...
        try:
            ...
            if wallet_post_request.wallet_metadata:
                data_copy["wallet_metadata"] = (
                    wallet_post_request.wallet_metadata.model_dump()
                )
        ...
        trust_params = self.trust_evaluator.get_jwt_header_trust_parameters(
        ...
        alg_values_supported = (
            wallet_post_request.wallet_metadata.alg_values_supported
            if wallet_post_request.wallet_metadata
            else []
        ) or []

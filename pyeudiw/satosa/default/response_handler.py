...
from pyeudiw.openid4vp.nuovo_proposta import VpTokenVerifier
...
class ResponseHandler(ResponseHandlerInterface, BackendTrust):
    ...
    def response_endpoint(self, context: Context, *args: tuple) -> Redirect | JsonResponse:
        ...
        for vp_token in encoded_vps:
            # -- START HERE --
            nuovo_verifier: VpTokenVerifier = self._vp_verifier_factory(authz_payload.presentation_submission)
            verified_claims = nuovo_verifier.get_verifiable_credential()
    ...
    def _vp_verifier_factory(self, presentation_submission: dict, token: str) -> VpTokenVerifier:
